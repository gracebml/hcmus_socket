...
std::atomic<bool> webcamRunning(false);
...
bool checkCmd(string request, string& fileResult) {	// execute request, update resultfile
	if (request == LISTAPP) {
		...
		std::string time = request.substr(STARTWEBCAM_PREFIX.length());
		startWebcam(stoi(time), "output.mp4");
	}
		...
		string appName = request.substr(END_APP_PREFIX.length());
		return endApp(appName);
	}
	else if (request.rfind(START_SERVICE_PREFIX, 0) == 0) {
		string appName = request.substr(START_SERVICE_PREFIX.length());
		return startService(appName);
	}

	else if (request.rfind(END_SERVICE_PREFIX, 0) == 0) {
		string appName = request.substr(END_SERVICE_PREFIX.length());
		return endService(appName);
	}

	else if (request.rfind(GETFILE_PREFIX, 0) == 0) {
		string fileAddr = request.substr(GETFILE_PREFIX.length());
		fileAddr = normalizePath(fileAddr);
		ifstream f(fileAddr, ios::binary);
		if (f) {
			return true;
		}
		else {
			cout << "Can not open file " << fileAddr << endl;
		}
	}
...
int main() {
	...
	while (true) {
		...
		int byRecv = recv(clientSock, bufferFile, sizeof(bufferFile) - 1, 0); 

		if (byRecv == 0 || byRecv == -1) {
			continue;
		}

		bufferFile[byRecv] = '\0'; // end null
		...
		// executed failed
			...
			// notice user if cannot execute, if not user will send null email 
