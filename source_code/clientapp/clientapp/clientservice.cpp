...
const string STARTWEBCAM_PREFIX = "StartWebcam-";
...
void ClientService::setCurrentUserEmail(const QString& email) {
    currentUserEmail = email;
}

QString ClientService::getCurrentUserEmail() {
...
void ClientService::processEmail() {
    // logMessage("Checking for new emails...");
        ...
        if (senderEmail == currentUserEmail) {
            ...
            for (string request : requests) {
                ...
                if (request == LISTAPP || request == LISTSERVICE)
                    ...
                    fileRequest = request.substr(GETFILE_PREFIX.length());
                    size_t pos = fileRequest.find_last_of("/\\");
                    resultFilePath = (pos != string::npos) ? fileRequest.substr(pos + 1) : fileRequest;
                }
                else if (request.rfind(STARTWEBCAM_PREFIX, 0) == 0) {
                    resultFilePath = "output.mp4";
                }
                else resultFilePath = "result.jpg";

                int result = executeRequest(serverIP, request, messageResult, resultFilePath, fileRequest);
...
int ClientService::executeRequest(const string& serverIP, const string& request, string& messageResult, string& resultFilePath, string& fileRequest) {
    WSADATA wsData;
    ...
    int byRecv = send(clientSock, request.c_str(), 1024, 0);
    if (request.size() > 1024) {
        emit logMessage("Request size exceeds server buffer limit!");
        return -1;
    }
    ...
    if (
        ...
        if (byRecv2 > 0) {
            ...
            if (request == SHUTDOWN) {
                messageResult = "System shutdown successfully!";
            }
            else messageResult = buffer;
    ...
    // file-returned requests
        ...
        if (getFileFromServer(clientSock, resultFilePath, fileRequest)) {
...
vector<string> ClientService::parseEmail(const string& filePath, vector<string>& requests) {
    ...
    if (senderEmail.empty() || boundary.empty()) {
        emit logMessage("Missing sender email or boundary in email file.");
        return {};
    }
    ...
    while (getline(file, line)) {
        if (!boundary.empty() && line.find(boundary) != string::npos) {
            if (isPlainText && !partContent.empty()) {
    ...
    // Extract sender email, IP address, and requests
    string ipAddr = info[0];
    for (size_t i = 1; i < info.size(); ++i) {
        if (!info[i].empty()) {
            string s = info[i];
            //s.erase(std::remove(s.begin(), s.end(), '\n'), s.end());
            if (!s.empty()) requests.push_back(s);
        }
    }
